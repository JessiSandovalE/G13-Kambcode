...
function obtenerJugadoresPorSeleccion(codigoPais) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const jugadoresPorSeleccion = jugadores.filter(jugador => jugador.codigoPais === codigoPais);
      resolve(jugadoresPorSeleccion);
    }, 5000);
  });
}
...
obtenerSelecciones()
  ...
  .then(jugadoresPorSeleccion => {
    console.log("Jugadores obtenidos:", jugadoresPorSeleccion);
  })
  .catch(error => {
    console.error(error);
  });